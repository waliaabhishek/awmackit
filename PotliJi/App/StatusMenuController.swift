...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    private func ensureStatusItem(using settings: LinkRouterSettings? = nil) {
        ...
        item.menu = menu
        item.button?.toolTip = "PotliJi — Link Router"
        statusItem = item
        updateButtonAppearance(using: settings)
    }
    ...
    private func rebuildMenu() {
        ...
        let isDefaultBrowser = DefaultBrowserManager().isPotliJiDefaultBrowser
        let defaultItem = NSMenuItem(
            title: isDefaultBrowser ? "PotliJi Is the Default Browser" : "Use PotliJi as Default Browser",
        ...
        menu.addItem(.separator())
        let quit = NSMenuItem(
            title: "Quit PotliJi", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        menu.addItem(quit)
    }
    ...
    @objc private func makeDefaultBrowser(_ sender: Any?) {
        Task { @MainActor in
            do {
                ...
                guard !manager.isPotliJiDefaultBrowser else { return }
                try await manager.setPotliJiAsDefaultBrowser()
            } catch {
    ...
    @objc private func openClipboardURL(_ sender: Any?) {
        ...
        let urls = LinkRouterServices.extractURLs(from: value)
    ...
    @objc private func cleanClipboardURL(_ sender: Any?) {
        guard let value = NSPasteboard.general.string(forType: .string),
            let url = LinkRouterServices.extractURLs(from: value).first
        else {
            NSSound.beep()
            return
        }
