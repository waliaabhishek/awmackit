...
struct DefaultBrowserManager {
    enum DefaultBrowserError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .missingBundleIdentifier:
                "PotliJi does not have a bundle identifier."
            case .requestFailed(let scheme, let underlying):
                "macOS could not make PotliJi the \(scheme.uppercased()) link handler. \(underlying.localizedDescription)"
            case .verificationFailed:
                "macOS completed the request, but PotliJi is not the handler for both HTTP and HTTPS links."
            }
        }
    }
    ...
    var isPotliJiDefaultBrowser: Bool {
    ...
    func setPotliJiAsDefaultBrowser() async throws {
        guard !isPotliJiDefaultBrowser else { return }
