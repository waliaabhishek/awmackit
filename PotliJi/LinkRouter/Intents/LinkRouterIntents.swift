...
struct OpenURLWithPotliJiIntent: AppIntent {
    static let title: LocalizedStringResource = "Open URL with PotliJi"
    static let description = IntentDescription(
        "Route one or more URLs through PotliJi rules, profiles, privacy cleaning, and native-app routing.")
...
struct OpenClipboardURLIntent: AppIntent {
    ...
    static let description = IntentDescription("Find web links on the clipboard and route them through PotliJi.")
    static let openAppWhenRun = false
    ...
    func perform() async throws -> some IntentResult {
        let urls = await MainActor.run { () -> [URL] in
            ...
            return LinkRouterServices.extractURLs(from: text)
        }
...
struct SetPrimaryBrowserIntent: AppIntent {
    ...
    static let description = IntentDescription("Change the primary browser or browser profile used by PotliJi.")
    static let openAppWhenRun = false
