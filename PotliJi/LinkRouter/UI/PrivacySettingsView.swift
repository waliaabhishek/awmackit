    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: SettingsDesign.pageSpacing) {
                SettingsPageHeader(
                    title: "Privacy",
                    subtitle: "Keep links cleaner and control what PotliJi remembers."
                )
                ...
                linkProtectionCard
                    .alert("Allow Automatic Clipboard Cleaning?", isPresented: $confirmsClipboardMonitoring) {
                        ...
                        Text(
                            "PotliJi will check newly copied text for web links. On current macOS releases, the system asks before allowing the first matching clipboard read. Nothing is uploaded."
                        )
                    }
                historyCard
                advancedCard
    ...
    private func enableClipboardCleaning() {
        guard !clipboardAccessIsDenied else {
            ...
            alert.informativeText =
                "Allow PotliJi under System Settings > Privacy & Security > Pasteboard, then turn automatic cleaning on again."
...
private struct AdvancedPrivacySettingsView: View {
    ...
    private var trackingOverridesCard: some View {
        SettingsSectionCard("Tracking Overrides") {
            ...
            SettingsControlRow(
                title: "Always keep",
                detail: "Required parameters that PotliJi must never remove.",
    ...
    private var shortenedLinksCard: some View {
        SettingsSectionCard("Shortened Links") {
            SettingsControlRow(
                ...
                detail:
                    "Stops after this many trusted shortener-to-shortener hops. The final destination is opened by your browser, not fetched by PotliJi.",
