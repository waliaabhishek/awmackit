...
struct SettingsRootView: View {
    ...
    @AppStorage(AppIdentity.selectedSettingsPaneKey) private var selectedPaneID = Pane.general.rawValue
    @State private var selectedPane: Pane = {
        let stored = UserDefaults.standard.string(forKey: AppIdentity.selectedSettingsPaneKey)
...
private struct GeneralSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: SettingsDesign.pageSpacing) {
            SettingsPageHeader(
                title: "General",
                subtitle: "Manage PotliJi, its menu-bar presence, and system link-handler setup."
            )
                ...
                Section("Link Router") {
                    ...
                    SettingsControlRow(
                        title: isDefaultBrowser
                            ? "PotliJi is the system link handler"
                            : "PotliJi is not the system link handler",
    ...
    private func routeTestLink() {
        guard let url = URL(string: "https://example.com/?utm_source=potliji-setup") else { return }
