    ...
    var body: some View {
        Form {
            Section("Link Cleaning") {
                Toggle("Remove known tracking parameters", isOn: setting(\.removeTrackingParameters))
                Toggle("Unwrap embedded redirect links", isOn: setting(\.unwrapEmbeddedRedirects))
                Toggle("Expand recognized short links", isOn: setting(\.expandShortURLs))
                Toggle("Resolve unknown redirect services over the network", isOn: setting(\.resolveUnknownRedirects))
                    .help(
                        "Sends a HEAD request to unknown short-link hosts. Leave this off for strictly local processing."
                    )
                Toggle(
                    "Clean web links copied to the clipboard",
                    isOn: Binding(
            ...
            Section("Local History") {
                Toggle("Keep link history", isOn: setting(\.historyEnabled))
                Stepper(
