...
private enum SettingsInitialLoadResult: Sendable {
    case success(PowerToolsSettings)
    case failure(String)
}
...
final class SettingsStore: ObservableObject {
    @Published var settings: PowerToolsSettings {
        didSet { scheduleSave() }
    }
    ...
    init(fileManager: FileManager = .default, settingsURL explicitURL: URL? = nil) {
        ...
        let defaultDirectory = applicationSupport.appendingPathComponent("PowerTools", isDirectory: true)
        ...
        initialLoadState = .loading(
            Task.detached(priority: .userInitiated) {
                do {
                    return .success(try Self.load(from: resolvedURL) ?? PowerToolsSettings())
                } catch {
    ...
    func loadIfNeeded() async {
        ...
        case .failure(let message):
            loadIssue = SettingsLoadIssue(
                message: "Power Tools did not overwrite the unreadable settings file. \(message)",
                fileURL: settingsURL
            )
        }
    }
    ...
    private nonisolated static func load(from url: URL) throws -> PowerToolsSettings? {
        guard FileManager.default.fileExists(atPath: url.path) else { return nil }
        let data = try BoundedFileReader.read(from: url, maximumBytes: RuleTransfer.maximumDocumentBytes)
        return try decodeSettings(from: data)
    }

    private nonisolated static func decodeSettings(from data: Data) throws -> PowerToolsSettings {
        ...
        let defaultsData = try encoder.encode(PowerToolsSettings())
        ...
        let decoded = try decoder.decode(PowerToolsSettings.self, from: mergedData)
        guard decoded.schemaVersion <= PowerToolsSettings.currentSchemaVersion else {
...
private actor SettingsPersistenceWriter {
    func write(_ settings: PowerToolsSettings, to url: URL) throws {
