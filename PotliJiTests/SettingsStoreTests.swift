...
final class SettingsStoreTests: XCTestCase {
    func testNewInstallPermissionSensitiveDefaults() {
        let settings = PotliJiSettings().linkRouter
    ...
    func testNewerSchemaIsNotOverwrittenOnSave() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let settingsURL = directory.appendingPathComponent("settings.json")
        var futureSettings = PotliJiSettings()
        futureSettings.schemaVersion = PotliJiSettings.currentSchemaVersion + 1
    ...
    func testRedirectLimitDefaultsToTenWhenMissingFromStoredSettings() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let settingsURL = directory.appendingPathComponent("settings.json")
        let encodedDefaults = try JSONEncoder().encode(PotliJiSettings())
    ...
    func testVersionFourUnsafeRedirectOptionsAreRetiredOnMigration() async throws {
        ...
        XCTAssertEqual(store.settings.schemaVersion, PotliJiSettings.currentSchemaVersion)
    ...
    func testVersionOneAlternativeRulesMigrateToTheirConfiguredBrowser() async throws {
        ...
        var oldSettings = PotliJiSettings()
    ...
    private func makeTemporaryDirectory() throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PotliJiTests-\(UUID().uuidString)", isDirectory: true)
